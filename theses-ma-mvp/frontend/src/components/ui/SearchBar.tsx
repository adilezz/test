...
export default function SearchBar() {
  ...
  // Show search history if no suggestions and query is empty
  const showHistory = query.length === 0 && state.searchHistory.length > 0;
  ...
  const handleSearch = (searchQuery: string) => {
    if (searchQuery.trim()) {
      ...
      dispatch({ type: 'ADD_TO_HISTORY', payload: searchQuery.trim() });
          ...
          <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-50 max-h-64 overflow-y-auto">
            {filteredSuggestions.length > 0 ? (
              ...
              <>
                ...
                {state.searchHistory.map((historyItem, index) => (
                  <button
                    key={index}
                    type="button"
                    onClick={() => {
